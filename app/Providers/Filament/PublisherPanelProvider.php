...
use App\Filament\Publisher\Pages\Import;
use App\Filament\Publisher\Pages\ImportPreview;
use App\Filament\Publisher\Pages\ImportConfirm;
...
class PublisherPanelProvider extends PanelProvider
{
    public function panel(Panel $panel): Panel
    {
        return $panel
            ...
            ->renderHook(PanelsRenderHook::HEAD_END,  fn(): HtmlString => new HtmlString(self::skeletonStyle()))
            ->renderHook(PanelsRenderHook::BODY_END, fn(): HtmlString => new HtmlString(self::tableResizerScript()))
            ->authMiddleware([Authenticate::class])
            ->authGuard('web');
    }

    private static function skeletonStyle(): string
    {
        return <<<'HTML'
<style>
/* ── Shimmer skeleton cho Filament table (deferLoading) ──────────────────── */
@keyframes fi-shimmer {
    0%   { background-position: 200% center; }
    100% { background-position: -200% center; }
}

/* TextColumn, BadgeColumn, IconColumn skeleton cells */
.fi-ta-table tbody .animate-pulse {
    animation: fi-shimmer 1.5s ease-in-out infinite !important;
...
/* Alternating row — skeleton shade nhạt hơn một chút */
.fi-ta-table tbody tr:nth-child(even) .animate-pulse {
    background: linear-gradient(90deg, #ebebee 25%, #f5f5f7 50%, #ebebee 75%) !important;
    background-size: 200% 100% !important;
}

/* Đảm bảo skeleton cell có chiều cao tối thiểu */
.fi-ta-table tbody td .animate-pulse {
    min-height: 1rem;
    display: block;
}
</style>
