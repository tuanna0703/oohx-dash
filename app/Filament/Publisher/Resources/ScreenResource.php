    ...
    public static function form(Form $form): Form
    {
        ...
        return $form->schema([
            ...
            Forms\Components\Section::make()->schema([
                ...
                // Screen count override — override số màn hình vật lý tại location
            ...
            Forms\Components\Section::make('Frequency cap')
                ...
                ->visible($canPricing)
                ->schema([
                    // Đã thêm vào DB — bỏ dehydrated(false), đổi prefix thành inventory.*
        ...
        return $table
            ...
            ->filters([
                ...
                SelectFilter::make('site_id')
                    ...
                    ->options(function () {
                        $ownerId = auth()->user()?->current_owner_id;
                        return cache()->remember("filter_sites_{$ownerId}", 300, fn() =>
                            \App\Models\Site::where('owner_id', $ownerId)
                                ->orderBy('name')
                                ->pluck('name', 'id')
                                ->toArray()
                        );
                ...
                SelectFilter::make('network')
                    ...
                    ->options(function () {
                        $ownerId = auth()->user()?->current_owner_id;
                        return cache()->remember("filter_networks_{$ownerId}", 300, fn() =>
                            \App\Models\Network::where('owner_id', $ownerId)
                                ->orderBy('name')
                                ->pluck('name', 'id')
                                ->toArray()
                        );
                ...
                SelectFilter::make('venue_type')
                    ...
                    ->options(fn() => cache()->remember('filter_venue_types', 600, fn() =>
                        \App\Models\VenueType::forSelect()
                            ->get()
                            ->pluck('venue_type', 'venue_type')
                            ->toArray()
                    ))
                    ->query(fn(Builder $q, array $data) => $q->when(
                        $data['value'] ?? null,
